...
add_action( 'after_setup_theme', 'soul_woocommerce_support' );
if ( ! function_exists( 'soul_woocommerce_support' ) ) {
	/**
	 * Declares WooCommerce theme support.
	 */
	function soul_woocommerce_support() {
		...
		add_filter( 'woocommerce_form_field_args', 'soul_wc_form_field_args', 10, 3 );
		add_filter( 'woocommerce_form_field_radio', 'soul_wc_form_field_radio', 10, 4 );
		add_filter( 'woocommerce_quantity_input_classes', 'soul_quantity_input_classes' );
		add_filter( 'woocommerce_loop_add_to_cart_args', 'soul_loop_add_to_cart_args' );

		// Wrap the add-to-cart link in `div.add-to-cart-container`.
		add_filter( 'woocommerce_loop_add_to_cart_link', 'soul_loop_add_to_cart_link' );

		// Add Bootstrap classes to account navigation.
		add_filter( 'woocommerce_account_menu_item_classes', 'soul_account_menu_item_classes' );
	}
}
...
add_action( 'woocommerce_before_main_content', 'soul_woocommerce_wrapper_start', 10 );
add_action( 'woocommerce_after_main_content', 'soul_woocommerce_wrapper_end', 10 );

if ( ! function_exists( 'soul_woocommerce_wrapper_start' ) ) {
	/**
	 * Display the theme specific start of the page wrapper.
	 */
	function soul_woocommerce_wrapper_start() {
...
if ( ! function_exists( 'soul_woocommerce_wrapper_end' ) ) {
	/**
	 * Display the theme specific end of the page wrapper.
	 */
	function soul_woocommerce_wrapper_end() {
		echo '</main>';
...
if ( ! function_exists( 'soul_wc_form_field_args' ) ) {
	/**
	 ...
	 */
	function soul_wc_form_field_args( $args, $key, $value ) {
		$bootstrap4 = 'bootstrap4' === get_theme_mod( 'soul_bootstrap_version', 'bootstrap4' );
...
if ( ! function_exists( 'soul_wc_form_field_radio' ) ) {
	/**
	 ...
	 * `soul_wc_form_field_args()` that is meant for input labels only.
	 *
	 ...
	 * @phpstan-param T $args
	 */
	function soul_wc_form_field_radio( $field, $key, $args, $value ) {
...
if ( ! function_exists( 'soul_quantity_input_classes' ) ) {
	/**
	 ...
	 */
	function soul_quantity_input_classes( $classes ) {
		$classes[] = 'form-control';
		return $classes;
	}
}

if ( ! function_exists( 'soul_loop_add_to_cart_link' ) ) {
	/**
	 ...
	 */
	function soul_loop_add_to_cart_link( $html ) {
		return '<div class="add-to-cart-container">' . $html . '</div>';
	}
}

if ( ! function_exists( 'soul_loop_add_to_cart_args' ) ) {
	/**
	 ...
	 */
	function soul_loop_add_to_cart_args( $args ) {
...
if ( ! function_exists( 'soul_account_menu_item_classes' ) ) {
	/**
	 ...
	 */
	function soul_account_menu_item_classes( $classes ) {
