...
if ( ! function_exists( 'soul_add_widget_categories_class' ) ) {
	/**
	 ...
	 */
	function soul_add_widget_categories_class( $cat_args ) {

		if ( isset( $cat_args['class'] ) ) {
			$cat_args['class'] .= ' ' . soul_get_select_control_class();
		} else {
			$cat_args['class'] = soul_get_select_control_class();
		}
...
add_filter( 'widget_categories_dropdown_args', 'soul_add_widget_categories_class' );

if ( ! function_exists( 'soul_add_block_widget_categories_class' ) ) {
	/**
	 ...
	 */
	function soul_add_block_widget_categories_class( $output, $parsed_args ) {
		$class = soul_get_select_control_class();
...
add_filter( 'wp_dropdown_cats', 'soul_add_block_widget_categories_class', 10, 2 );

if ( ! function_exists( 'soul_add_block_widget_archives_classes' ) ) {
	/**
	 ...
	 */
	function soul_add_block_widget_archives_classes( $block_content, $block ) {

		if ( isset( $block['attrs']['displayAsDropdown'] ) && true === $block['attrs']['displayAsDropdown'] ) {
			return str_replace(
				'<select',
				'<select class="' . soul_get_select_control_class() . '"',
				$block_content
			);
		}
...
add_filter( 'render_block_core/archives', 'soul_add_block_widget_archives_classes', 10, 2 );

if ( ! function_exists( 'soul_add_block_widget_search_classes' ) ) {
	/**
	 ...
	 * @return string The filtered block content.
	 */
	function soul_add_block_widget_search_classes( $block_content, $block ) {
...
add_filter( 'render_block_core/search', 'soul_add_block_widget_search_classes', 10, 2 );
 ...
 */
function soul_hero_active_carousel_item( $params ) {
	if (
		! isset( $params[0] )
		|| ! isset( $params[0]['id'] )
		|| 'hero' !== $params[0]['id']
...
add_filter( 'dynamic_sidebar_params', 'soul_hero_active_carousel_item' );
 ...
 */
add_filter( 'dynamic_sidebar_params', 'soul_widget_classes' );

if ( ! function_exists( 'soul_widget_classes' ) ) {
	 ...
	 * @return array Maybe filtered parameters.
	 */
	function soul_widget_classes( $params ) {
...
} // End of if function_exists( 'soul_widget_classes' ).

add_action( 'widgets_init', 'soul_widgets_init' );

if ( ! function_exists( 'soul_widgets_init' ) ) {
	/**
	 * Initializes themes widgets.
	 */
	function soul_widgets_init() {
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Right Sidebar', 'soul' ),
				'id'            => 'right-sidebar',
				'description'   => __( 'Right sidebar widget area', 'soul' ),
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Left Sidebar', 'soul' ),
				'id'            => 'left-sidebar',
				'description'   => __( 'Left sidebar widget area', 'soul' ),
				'before_widget' => '<aside id="%1$s" class="widget %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h3 class="widget-title">',
				'after_title'   => '</h3>',
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Hero Slider', 'soul' ),
				'id'            => 'hero',
				'description'   => __( 'Hero slider area. Place two or more widgets here and they will slide!', 'soul' ),
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Hero Canvas', 'soul' ),
				'id'            => 'herocanvas',
				'description'   => __( 'Full size canvas hero area for Bootstrap and other custom HTML markup', 'soul' ),
				'before_widget' => '',
				'after_widget'  => '',
				'before_title'  => '',
				'after_title'   => '',
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Top Full', 'soul' ),
				'id'            => 'statichero',
				'description'   => __( 'Full top widget with dynamic grid', 'soul' ),
		...
		register_sidebar(
			array(
				/* translators: Widget area title */
				'name'          => __( 'Footer Full', 'soul' ),
				'id'            => 'footerfull',
				'description'   => __( 'Full sized footer widget with dynamic grid', 'soul' ),
...
} // End of function_exists( 'soul_widgets_init' ).
